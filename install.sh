...
if ((INSTALL_PACKAGES)); then
    ...
    apt-get install -y --no-install-recommends \
        ...
        fontconfig fonts-dejavu-core fonts-noto-core fonts-noto-cjk libfuse2 fuse3 rclone openssh-client
...
payload.setdefault("mapping_rpc_timeout_seconds", 90)
payload.setdefault("mapping_provider_idle_timeout_seconds", 60)
...
# Generate a Caddy route fragment from the configured base path. The installer
# does not edit or reload an existing reverse proxy because its site layout and
# process management are host-specific.
CADDY_ROUTES_FILE="$DATA_DIR/caddy-routes.caddyfile"
"$INSTALL_DIR/venv/bin/python" - "$CONFIG_FILE" >"$CADDY_ROUTES_FILE" <<'PY'
import json
import sys

payload = json.load(open(sys.argv[1], encoding="utf-8"))
base = payload.get("url_base_path", "/kapsel").rstrip("/") or "/"
port = int(payload.get("listen_port", 8765))
api_match = base + "*" if base == "/" else base + "/*"
authorization_metadata = "/.well-known/oauth-authorization-server" + base.rstrip("/") + "/oauth/*"
resource_metadata = "/.well-known/oauth-protected-resource" + base.rstrip("/") + "/connect/*"
print("# Generated by OpenKapsel install.sh; import this inside the API-origin site block.")
print(f"@openkapsel_oauth_metadata path {authorization_metadata} {resource_metadata}")
print("handle @openkapsel_oauth_metadata {")
print(f"    reverse_proxy 127.0.0.1:{port}")
print("}")
print()
print(f"handle {api_match} {{")
print(f"    reverse_proxy 127.0.0.1:{port}")
print("}")
PY
chown root:root "$CADDY_ROUTES_FILE"
chmod 0644 "$CADDY_ROUTES_FILE"
ADMIN_URL_PATH=$("$INSTALL_DIR/venv/bin/python" - "$CONFIG_FILE" <<'PY'
import json
import sys

payload = json.load(open(sys.argv[1], encoding="utf-8"))
base = payload.get("url_base_path", "/kapsel").rstrip("/")
print((base or "") + "/admin")
PY
)
...
if ((START_SERVICE)); then
    ...
    printf 'OpenKapsel is active. Admin URL path: %s\n' "$ADMIN_URL_PATH"
    printf 'Caddy route fragment: %s (merge or import it, then validate and reload Caddy)\n' "$CADDY_ROUTES_FILE"
else
    printf 'Installation completed without starting OpenKapsel.\n'
    printf 'Caddy route fragment: %s (merge or import it before exposing the service)\n' "$CADDY_ROUTES_FILE"
fi
