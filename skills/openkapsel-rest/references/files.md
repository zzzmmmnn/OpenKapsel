...
`fs/mutate` is the single ordinary mutation protocol. Every existing target must carry the exact ETag observed by the preceding stat/read/search; wildcard ETags are rejected. `file.create` is create-only. `text.replace` evaluates exact replacement rules only inside its selected range and requires the declared occurrence count there. Each boundary can use a zero-based inclusive line selector or a full-file-unique `start_text` / `end_text` marker that may contain multiple lines. Text marker bounds are inclusive: the range starts at the first character of `start_text` and ends immediately after the final character of `end_text`, so the marker text itself may be replaced. Text and line selectors are mutually exclusive on the same side. If omitted, the start defaults to line 0 and the end to EOF. `structured.patch` supports guarded JSON/YAML/TOML edits. `path.delete` recycles files or directories and can include large files because it does not inspect their content.
