...
`fs/mutate` is the single ordinary mutation protocol. Every existing target must carry the exact ETag observed by the preceding stat/read/search; wildcard ETags are rejected. `file.create` is create-only. `text.replace` evaluates exact replacement rules only inside its optional zero-based inclusive `start_line` / `end_line` range and requires the declared occurrence count there; omitted `start_line` starts at 0 and omitted `end_line` runs through EOF. `structured.patch` supports guarded JSON/YAML/TOML edits. `path.delete` recycles files or directories and can include large files because it does not inspect their content.
