...
Static and OAuth connection pages group entries by workspace directory. Multiple connections can reference the same workspace configuration. Comments can be edited after creation. Static connections show their expiration and last authenticated use. Editing a comment leaves expiration unchanged by default. Choosing a duration resets expiration to the save time plus that duration, including for an expired connection; its URL and secret stay unchanged. Delete a connection to revoke it. Recreate it when a new secret is required.
