...
The private registry is `static-mcp.sqlite3` alongside `oauth.sqlite3`, normally under `/var/lib/openkapsel`. It has mode 0600 and is outside workspace sandbox mounts. Static secrets are retained so administrators can copy client JSON again. Protect its backups like the REST credentials database. Last-use timestamps are written at most once per minute. Authenticated Static MCP connections can also use `get_workspace_credentials` to export the linked configuration's current REST workspace URL/control token and `renew_workspace_credentials` to rotate that REST pair inside the normal final-two-day renewal window. Rotating REST credentials does not change the Static MCP secret or its expiration.
