...
from openkapsel.auth.tokens import PathGrant, TokenRecord, parse_datetime
from openkapsel.workspace.workspace_images import WorkspaceImage


STYLE = """
:root{color-scheme:light;--bg:#f5f5f7;--card:#fff;--text:#1d1d1f;--muted:#6e6e73;--line:#e5e5e9;--field:#fff;--field-line:#d2d2d7;--subtle:#f5f5f7;--hover:#f5f5f7;--sidebar:#ececf0;--sidebar-text:#424245;--sidebar-muted:#6e6e73;--sidebar-hover:#dedee3;--sidebar-active:#d9e8fb;--brand:#007aff;--brand-text:#fff;--danger:#d92d3e;--secondary:#e9f2ff;--secondary-text:#075bb6;--danger-bg:#fff0f1;--danger-line:#f3c9d0;--success-bg:#eaf7ed;--success-text:#1b713a;--success-line:#c8e7d0;--notice-bg:#fff8e4;--notice-line:#f0dfb2;--shadow:0 10px 34px #1d1d1f0a}
:root[data-theme=dark]{color-scheme:dark;--bg:#1c1c1e;--card:#2c2c2e;--text:#f5f5f7;--muted:#a1a1a6;--line:#444448;--field:#3a3a3c;--field-line:#5a5a5e;--subtle:#353537;--hover:#38383b;--sidebar:#242426;--sidebar-text:#e5e5ea;--sidebar-muted:#a1a1a6;--sidebar-hover:#38383c;--sidebar-active:#173e68;--brand:#0a84ff;--brand-text:#fff;--danger:#ff6576;--secondary:#163d66;--secondary-text:#b6d9ff;--danger-bg:#4a272d;--danger-line:#784048;--success-bg:#203d2c;--success-text:#a7e3b8;--success-line:#315b40;--notice-bg:#443a25;--notice-line:#69572b;--shadow:0 10px 34px #00000025}
...
h1{font-size:25px;margin:0;letter-spacing:-.025em}h2{font-size:18px;margin:0 0 16px;letter-spacing:-.015em}.muted{color:var(--muted)}.card{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:20px;margin-bottom:18px;box-shadow:var(--shadow)}
...
label{display:block;font-weight:600;margin-bottom:5px}input,select,textarea{width:100%;border:1px solid var(--field-line);border-radius:10px;padding:9px 10px;background:var(--field);color:var(--text)}input::placeholder,textarea::placeholder{color:var(--muted)}input:focus-visible,select:focus-visible,textarea:focus-visible,button:focus-visible,summary:focus-visible{outline:2px solid var(--brand);outline-offset:2px}textarea{min-height:76px;resize:vertical;font:13px/1.4 ui-monospace,SFMono-Regular,monospace}
...
button,.button{border:0;border-radius:10px;padding:9px 13px;background:var(--brand);color:var(--brand-text);font-weight:650;cursor:pointer;text-decoration:none;display:inline-block;font:inherit}button:disabled{opacity:.5;cursor:not-allowed}
button.secondary{background:var(--secondary);color:var(--secondary-text)}button.danger{background:var(--danger-bg);color:var(--danger);border:1px solid var(--danger-line)}.actions{display:flex;gap:8px;flex-wrap:wrap;align-items:center}
.token{font:12px/1.4 ui-monospace,SFMono-Regular,monospace;background:var(--subtle);border:1px solid var(--line);padding:9px;border-radius:10px;word-break:break-all;margin:10px 0;color:var(--text)}
.badge{display:inline-block;border-radius:99px;padding:3px 8px;font-size:12px;font-weight:700;background:var(--success-bg);color:var(--success-text)}.badge.off{background:var(--danger-bg);color:var(--danger)}
...
.login{max-width:420px;margin:12vh auto}.login-main{display:flex;align-items:center;justify-content:center;width:100%;max-width:none;min-width:0;margin:0;padding:32px}.login-main .login{width:100%;max-width:420px;margin:0}.login .card{padding:30px}.login .card h1{margin-bottom:7px}.login .card button[type=submit]{width:100%}.error{background:var(--danger-bg);border:1px solid var(--danger-line);color:var(--danger);padding:10px;border-radius:10px;margin-bottom:14px}
.success{background:var(--success-bg);border:1px solid var(--success-line);color:var(--success-text);padding:10px;border-radius:10px;margin-bottom:14px}
.notice{background:var(--notice-bg);border:1px solid var(--notice-line);padding:10px;border-radius:10px;margin-top:12px}.token-card{border-left:4px solid var(--line)}.token-card.invalid{border-left-color:var(--danger)}
...
.admin-shell{display:grid;grid-template-columns:220px minmax(0,1fr);min-height:100vh}.admin-sidebar{position:sticky;top:0;height:100vh;background:var(--sidebar);color:var(--sidebar-text);padding:22px 14px;display:flex;flex-direction:column;gap:22px;overflow:hidden;border-right:1px solid var(--line)}.admin-brand{display:flex;align-items:center;gap:11px;padding:0 9px}.brand-mark{display:grid;place-items:center;flex:0 0 34px;height:34px;border-radius:10px;background:var(--brand);color:#fff;font-size:17px;font-weight:800}.brand-copy{min-width:0}.brand-copy strong,.brand-copy span{display:block;white-space:nowrap}.brand-copy span{color:var(--sidebar-muted);font-size:12px;margin-top:2px}.admin-nav{display:flex;flex-direction:column;gap:7px}.nav-item{width:100%;display:flex;align-items:center;gap:11px;background:transparent;color:var(--sidebar-text);padding:11px 12px;text-align:left;border-radius:10px;white-space:nowrap}.nav-item:hover{background:var(--sidebar-hover);color:var(--text)}.nav-item.active{background:var(--sidebar-active);color:var(--text)}.nav-icon{display:grid;place-items:center;flex:0 0 24px;font-size:16px}.sidebar-spacer{flex:1}.logout-button,.theme-button{background:transparent;color:var(--sidebar-text);font-weight:500}.theme-button{margin-bottom:4px}.admin-main{width:100%;max-width:1280px;margin:0 auto;padding:30px 34px}.admin-panel[hidden]{display:none}.panel-heading{margin-bottom:18px}.panel-heading h2{margin-bottom:4px}
details.token-card{padding:0;overflow:hidden}details.token-card>summary{list-style:none}details.token-card>summary::-webkit-details-marker{display:none}.token-summary{display:grid;grid-template-columns:minmax(220px,1.5fr) minmax(150px,1fr) minmax(180px,1fr) minmax(220px,1.2fr) auto;align-items:center;gap:16px;padding:17px 19px;cursor:pointer;user-select:none}.token-summary:hover{background:var(--hover)}.token-summary-title{font-size:16px;font-weight:750;min-width:0}.token-summary-title>span:first-child{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.token-summary-meta{min-width:0}.token-summary-meta strong,.token-summary-meta span{display:block}.token-summary-meta strong{font-size:12px;color:var(--muted);margin-bottom:2px}.token-summary-meta span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.summary-toggle{color:var(--brand);font-weight:700;white-space:nowrap}.summary-toggle::after{content:'Show settings'}.summary-toggle::before{content:'+';display:inline-block;margin-right:6px;font-size:18px;line-height:1}details[open] .summary-toggle::after{content:'Hide settings'}details[open] .summary-toggle::before{content:'−'}.token-details{border-top:1px solid var(--line);padding:20px}.token-details>.top{margin-bottom:14px}
...
<section id="panel-password" class="admin-panel" data-admin-panel="password"{password_hidden}><div class="panel-heading"><h2>Change password</h2><div class="muted">Update administration login credentials.</div></div><section class="card"><h2>Change administrator password</h2><form method="post" action="{escaped_admin_path}/password"><input type="hidden" name="csrf" value="{esc_csrf}"><div class="grid"><div><label for="old-password">Current password</label><input id="old-password" name="old_password" type="password" autocomplete="current-password" required></div><div><label for="new-password">New password</label><input id="new-password" name="new_password" type="password" autocomplete="new-password" minlength="12" required></div><div><label for="confirm-password">Repeat new password</label><input id="confirm-password" name="confirm_password" type="password" autocomplete="new-password" minlength="12" required></div><div class="checks"><button type="submit">Change password</button></div></div></form><p class="muted">The new password must contain at least 12 characters. Other administration sessions are invalidated immediately after a successful change.</p></section></section>
    ...
    from openkapsel.auth.oauth_ui import render_connections
    ...
    from openkapsel.mapping.mapping_ui import render_mappings
    ...
    body = body.replace('aria-hidden="true">↪</span>', 'aria-hidden="true">🚪</span>')
    body = body.replace(
        '<div class="sidebar-spacer"></div><form method="post"',
        '<div class="sidebar-spacer"></div><button type="button" class="nav-item theme-button" data-theme-toggle>'
        '<span class="nav-icon" aria-hidden="true">🌙</span><span class="nav-label">Dark appearance</span>'
