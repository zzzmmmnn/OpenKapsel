...
from openkapsel.files.share_store import ShareError
...
class ShareHandlersMixin:
    def _handle_share_create(self) -> None:
        ...
        if self.server.mappings.at_path(path) is not None:
            from openkapsel.mapping.mapping_shares import create_share
            try:
    ...
    def _handle_share_import(self, share_id: str) -> None:
        ...
        if self.server.mappings.at_path(destination) is not None:
            from openkapsel.mapping.mapping_shares import import_share
            try:
