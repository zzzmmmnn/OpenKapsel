...
GIT_OPERATIONS = ("status", "diff", "log", "show", "ls_files", "diff_stat")
...
def git_arguments(operation, options):
    ...
    def revision(key, default=None):
        ...
        if value is not None and (not isinstance(value, str) or not value or len(value) > 256 or
                                  value.startswith("-") or any(ord(c) < 32 or 0xD800 <= ord(c) <= 0xDFFF for c in value)):
            raise ApiError(400, "invalid_request", f"{key} must be a bounded revision, not an option")
        return value

    paths = options.get("paths", [])
    if not isinstance(paths, list) or len(paths) > 100 or any(
        not isinstance(p, str) or not p or len(p) > 4096 or "\x00" in p or any(0xD800 <= ord(c) <= 0xDFFF for c in p) or
        p.startswith(("/", "\\")) or "\\" in p or ":" in p or ".." in p.split("/")
        for p in paths
    ) or sum(map(len, paths)) > 16000:
        raise ApiError(400, "invalid_request", "paths must be bounded literal repository-relative paths")
    argv = ["git", "--no-pager", "--no-optional-locks", "--literal-pathspecs",
    ...
    if operation == "status":
        ...
        argv += ["log", "--no-show-signature", "--format=%H%x09%aI%x09%an%x09%s",
                 f"--max-count={options.get('limit', 20)}", f"--skip={options.get('skip', 0)}",
                 revision("revision", "HEAD")]
    elif operation == "show":
        argv += ["show", "--no-show-signature", "--no-ext-diff", "--no-textconv", "--no-renames",
...
def git_tool_properties(operation):
    ...
    if operation in {"diff", "diff_stat"}:
        properties.update(to_revision={"type": "string", "maxLength": 256}, staged={"type": "boolean"})
    if operation == "log":
        properties.update(limit={"type": "integer", "minimum": 1, "maximum": 200, "default": 20},
