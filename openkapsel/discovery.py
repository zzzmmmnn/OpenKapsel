    ...
    def _full_discovery(self) -> dict[str, Any]:
        ...
        payload["capabilities"]["mappings"] = {
            ...
            "rpc": {
                ...
                "configuration": "Client config rpc.<family>=true|false selectively enables implemented families. Missing local dependencies are unsupported, not disabled. Plugin families self-describe with description plus operation_specs.<operation>.description/input_schema/write in GET /mappings; write is authoritative per operation.",
                "families": {
                    ...
                    "archive": {"version": 1, "fallback": "none", "operations": ["list", "read"],
        ...
        payload["endpoints"].update({
            ...
            "mapping_list": {"method": "GET", "url": "./mappings", "description": "List mapping IDs, roots, online/write state, and client capabilities. RPC plugins include family descriptions plus per-operation descriptions, JSON input schemas, and write flags for dynamic callers."},
            "mapping_rpc": {"method": "POST", "url": "./mappings/<mapping_id>/rpc/<family>/<operation>",
                "body": {"args": "<plugin-specific object>", "plan_id": "required when operation write=true", "taskname": "required when operation write=true", "message": "required when operation write=true"},
                "description": "Invoke one advertised client RPC plugin operation. write=false requires read permission. write=true requires Bearer control authorization, token write permission, a writable mapping, and Plan Context. No generic RPC operation falls back to server/FUSE."},
