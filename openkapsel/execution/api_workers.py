...
from openkapsel.execution.network_proxy import DomainProxy, PROXY_MOUNT, PROXY_PORT
from openkapsel.auth.tokens import TokenRecord
from openkapsel.mapping.mapping_api import MappingApiMixin
...
class ApiWorkerManager(MappingApiMixin):
    ...
    def _start(
        ...
        if self.cgroups.available:
            ...
            argv = [
                sys.executable,
                "-m",
                "openkapsel.execution.cgroup_exec",
                str(procs_file),
                *argv,
            ]
        try:
    ...
    def _stop_process(process):
        ...
        if getattr(process, "pid", None) is not None:
            import signal
            from openkapsel.mapping.mapping_process import group_alive, signal_group
